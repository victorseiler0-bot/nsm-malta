...
async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    const { title, description, creator_id, choice1_label, choice2_label, cote_for, cote_against, cote_null, badge_emoji, badge_name } = req.body || {}
    ...
    await withDB((data) => {
      created = {
        ...
        badge_name: badge_name?.trim() || title.trim(),
